...
This project implements a grounded long-term memory system that converts GitHub repository activity into a structured, queryable memory graph. The system extracts entities, relationships (claims), and evidence from issues, pull requests, comments, and timeline events. The resulting graph allows reliable retrieval of project knowledge while maintaining traceability to original source artifacts.

The design focuses on four core properties: grounding, reproducibility, deduplication, and maintainability over time.
...
GitHub API → Raw Artifacts → Normalization → Claim Extraction → Claim Deduplication → Current State Materialization → Memory Graph Construction → Visualization
...
The extraction pipeline follows a strict contract to ensure consistency.
